...
const errorDefinitions: { [key in ApiError]: ErrorDefinition } = {
    // Generic
    [ApiError.Unexpected           ]: err(500, 'An unexpected issue occured. Please try again later'),
    ...
    // Delete transaction
    [ApiError.NoDeleteTransaction  ]: err(405, "Transactions cannot be deleted, please use PATCH and the 'removed' flag"),
...
export function unexpectedErrorPleaseReport(details: string) {
    return err(
        500,
...
export function isErrorResolvable(
    maybeError: any
): maybeError is ErrorResolvable {
